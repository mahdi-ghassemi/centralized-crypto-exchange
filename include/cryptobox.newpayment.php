...
function cryptobox_new_payment($paymentID, $payment_details, $box_status)
{
	...
	if (!$processed && $payment_details["confirmed"])
	{
        ...
        $datas['customer_invoice_number'] = intval($paymentID);
        $datas['is_delete'] = "0";
        $order_id = validate_input($payment_details["order"]);
        update_order($order_id,$datas);
        $res = send_mail($no_reply_mail_address,$to_mail,$no_reply_mail_address,$no_reply_mail_password,'GoUrl Pay Confirmed','order id: '.$order_id,null);
