		...
		$_POST["box"] && is_numeric($_POST["box"]) && $_POST["box"] > 0 && $_POST["amount"] && is_numeric($_POST["amount"]) && $_POST["amount"] > 0 && $valid_key)
{
	...
	if (!$paymentID)
	{
        ...
        $datas = array();
        $datas['status'] = "2";
        $datas['customer_invoice_number'] = $paymentID;
        update_order($order_id,$datas);
        $res = send_mail($no_reply_mail_address,$to_mail,$no_reply_mail_address,$no_reply_mail_password,'GoUrl Pay Created','order id: '.$order_id,null);
