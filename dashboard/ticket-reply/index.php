...
if(isset($_SESSION['bitex_sessionid']) && isset($_SESSION['user_id']) && $_SESSION['bitex_sessionid'] === md5($_SESSION['user_id'])) {
    $user_login = true;
    insert_user_action($_SESSION['ip_address'],$_SESSION['user_id'],'ورود به پاسخ تیکت',$_SESSION['bitex_username']);
}
...
$ticket_id = "";
$ticket_info = array();
if(isset($_SESSION['reply_ticket_id']) && !empty($_SESSION['reply_ticket_id'])) {
    $ticket_id = $_SESSION['reply_ticket_id'];  
    $ticket_info = get_ticket_info_by_id($ticket_id);
    $ticket_files = array();
    $ticket_files = get_ticket_files($ticket_id);
...
<head>
    ...
    <script src="<?php echo $root_path; ?>asset/js/ticket-reply.js?ver=<?php echo $version;?>"></script>
        ...
        <section id="home">
            <div class="uk-form-stacked uk-grid-small uk-child-width-1-1 uk-child-width-1-@s home-content" uk-grid="">
                ...
                <div class="uk-card uk-card-default card-new-ticket">
                    ...
                    <div class="uk-card-body">
                        <div>
                            ...
                            <input type="text" value="<?php echo $ticket['subject']; ?>" class="uk-input" readonly>
                        </div>
                        <br>
                        <div>
                            <label class="uk-label shabnam-light">توضیحات</label>
                            <textarea class="uk-textarea" style="height:auto;" readonly><?php echo $ticket['description']; ?></textarea>
                        </div>
                ...
                <div class="uk-card uk-card-default card-new-ticket">
                    ...
                    <input id="t_id" value="<?php echo $ticket_id; ?>" type="hidden">
                    <div class="uk-card-footer uk-text-center">
                        <button id="submit" class="uk-button uk-button-primary uk-button-small new_order_btn"><i class="fa fa-spinner fa-spin spinner-onload"></i> ارسال تیکت </button>
                    </div>
                </div>
                <div>
