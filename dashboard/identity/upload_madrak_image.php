<?php
/*
  This is a ***DEMO*** , the backend / PHP provided is very basic. You can use it as a starting point maybe, but ***do not use this on production***. It doesn't preform any server-side validation, checks, authentication, etc.
  For more read the README.md file on this folder.
  Based on the examples provided on:
  - http://php.net/manual/en/features.file-upload.php
*/
session_start();
include_once('../../include/bitex.php');
include_once('../../include/jdf.php');
include_once('../../include/global.php');
include_once('../../include/mail.php');
header('Content-type:application/json;charset=utf-8');
try {
    if (
        !isset($_FILES['file']['error']) ||
        is_array($_FILES['file']['error'])
    ) {
        throw new RuntimeException('Invalid parameters.');
    }
    switch ($_FILES['file']['error']) {
        case UPLOAD_ERR_OK:
            break;
        case UPLOAD_ERR_NO_FILE:
            throw new RuntimeException('No file sent.');
        case UPLOAD_ERR_INI_SIZE:
        case UPLOAD_ERR_FORM_SIZE:
            throw new RuntimeException('Exceeded filesize limit.');
        default:
            throw new RuntimeException('Unknown errors.');
    }
    $file_new_name = uniqid().'_'.$_FILES['file']['name'];
    $filepath ='../../temp-files/'.$file_new_name;
    if (!move_uploaded_file($_FILES['file']['tmp_name'],$filepath)) {
        throw new RuntimeException('Failed to move uploaded file.');
    } else {
        $targetPath = "../../docs/".$_SESSION['bitex_username']."/";
        if(!file_exists($targetPath))
            mkdir($targetPath);
        $targetFile =  $targetPath. $file_new_name; 
        if (rename($filepath,$targetFile)){
            $datas = array();
            $datas['user_id'] = $_SESSION['user_id'];            
            $datas['url'] = "/docs/".$_SESSION['bitex_username']."/".$file_new_name;
            $datas['confirm'] = "0";
            $datas['confirm_user_id'] = "0";            
            $datas['insert_date'] = jdate('Y-m-d','','','','en');
            $datas['insert_time'] = jdate('H:i:s','','','','en'); 
            insert_madrak($datas);
            $res = send_mail_with_path($no_reply_mail_address,$to_mail,$no_reply_mail_address,$no_reply_mail_password,'New Document Uploaded','userid: '.$_SESSION['user_id'].' username: '.$_SESSION['bitex_username'],null,'../../');
        }
    }
    
    // All good, send the response
    echo json_encode([
        'status' => 'ok',
        'path' => $filepath
    ]);
} catch (RuntimeException $e) {    
	// Something went wrong, send the err message as JSON
	http_response_code(400);
	echo json_encode([
		'status' => 'error',
		'message' => $e->getMessage()
	]);
}
?>